...
# 在scheduler初始化时，会执行f(0)，即为epoch=0提供lr，之后由scheduler.step()出发f(epoch)
# return的似乎是optimizer.param_groups[0]['lr']即先前lr的的倍率gamma，以gamma * 原lr作为新的lr值，而非新一轮的lr值。。[注意是最初的lr的倍率！而非上一轮lr的倍率！！！]
def create_lr_scheduler(optimizer,epochs,lr_decay_step = 8,lr_decay_gamma = 0.7,warmup=True,warmup_epochs=4,warmup_factor=1e-4):
    assert epochs > 0
    if warmup is False:
        warmup_epochs = 0
    # x表示lr_scheduler.step()的次数
    def f(x):
        if warmup is True and x < warmup_epochs:
            # warmup中(线性递增)
            alpha = float(x+1) / warmup_epochs
            # return warmup_factor * (1 - alpha) + alpha
            # print("--------0,")
            return alpha
        else :
            # 每 lr_decay_step 轮 lr 衰减为初始 lr 的 lr_decay_gamma 倍
            decay_epochs = x - warmup_epochs
            return lr_decay_gamma ** (decay_epochs // lr_decay_step)
        return 1
...
def main(args):
    ...
    torch.cuda.manual_seed_all(args.manual_seed) #GPU设置随机种子，args.manual_seed是为了保证在每次运行时获得相同的随机数序列，是为了使实验结果可重复性。
    torch.manual_seed(args.manual_seed) #CPU设置随机种子
    np.random.seed(args.manual_seed) #设置NumPy库的随机数种子
    random.seed(args.manual_seed) #设置Python标准库中的随机数种子
    cudnn.benchmark = True #这一行代码用于启用CuDNN（CUDA深度神经网络库）的性能优化。CuDNN会在运行时根据输入数据的大小和类型自动优化卷积运算，从而提高深度神经网络的训练速度。
    ...
    if not args.evaluate:
        ...
        # 自定义scheduler
        # scheduler = create_lr_scheduler(optimizer,args.epochs-start_epoch,lr_decay_step = args.lr_decay_step,lr_decay_gamma = args.lr_decay_gamma,warmup_factor = args.lr)
        # print("-----after init scheduler")
        if args.use_ho3d:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if not args.evaluate:
            ...
            monitor.log_train(epoch + 1, {"Using lr ":optimizer.param_groups[0]["lr"]})
        ...
        if not args.evaluate:
            ...
            #             scheduler.step()
            # 调用f(epoch)，其中epoch>0，而epoch=0时的f(0)由scheduler初始化时调用
            scheduler.step()
