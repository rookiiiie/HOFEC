...
def single_epoch(loader, model, epoch=None, optimizer=None, save_path="checkpoints",
                 train=True, save_results=False, indices_order=None, use_cuda=False,scaler = None,autocast = None):
    ...
    for batch_idx, sample in enumerate(loader):
        if train:
            ...
            if scaler is not None:
                ...
                with autocast():
                    # model forward-------调用HOModel模型的forward
            ...
            # # 访问 preds_joints 中的第一个元素，即张量
            # preds_joints_tensor = preds_joints[0]
            # # 将 preds_joints_tensor 转换为张量类型
            # preds_joints_tensor = torch.tensor(preds_joints_tensor)
            # g = make_dot(preds_joints_tensor)
    ...
    if train:
        ...
        if save_results:
            ...
            # save_zip_path = os.path.join(save_path, "pred_epoch_{}.json".format(epoch+1) if epoch is not None else "pred_{}.json")
