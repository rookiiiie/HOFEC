...
def cam_equal_aspect_3d(ax, verts, flip_x=False):
    ...
    if flip_x:
        ...
        ax.set_xlim(centers[0] - r, centers[0] + r)
    # Invert y and z axis
    # ax.set_ylim(centers[0] + r, centers[0] - r)
    # ax.set_zlim(centers[2] + r, centers[2] - r)
    ax.set_zlim(centers[2] - r, centers[2] + r)
    ax.set_ylim(centers[1] - r, centers[1] + r) 
    # print(centers[0] + r, centers[0] - r)
    # print(centers[2] + r, centers[2] - r)
...
def plot3dVisualize(ax, m, flip_x=False, c="b", alpha=0.1, camPose=np.eye(4, dtype=np.float32), isOpenGLCoords=False):
    ...
    ax.view_init(-90, -90) # Show the figure, adjust it with the mouse 
                            #改变绘制图像的视角,即相机的位置,azim沿着z轴旋转，elev沿着y轴
    #ax.view_init(elev=0, azim=90)

    ax.plot_trisurf(list(verts[:,0]),list(verts[:,1]),faces,list(verts[:,2]))  #ax.plot_trisurf(xs, ys, zs, triangles=triangles, color=(0,0,0,0), edgecolor='Gray')
