...
class dex_ycb(data.Dataset):
    def __init__(self,dataset_root,mode,inp_res=256 ,max_rot=np.pi, scale_jittering=0.2, center_jittering=0.1,
        ...
        if self.mode == "train":
            ...
            # -----------------------------------自己测试：train只用20200709-subject-01这一个数据集进行训练！----------------------------------------------
            self.sample_list = {key: value for key, value in self.sample_dict.items() if value.get("color_file", "").startswith(("20200709-subject-01", "20200813-subject-02"))}
            # ------------------------------------------------------------------------------------------------------------------------------------------
            ...
            self.hand_bbox_list = []
            with open(os.path.join(dataset_root,"dex_ycb_s0_test_data_reduced.json"), 'r', encoding='utf-8') as f:
                self.sample_dict = json.load(f)
            self.sample_list = sorted(self.sample_dict.keys(),key=lambda x:int(x[3:]))

            # -----------------------------------自己测试：train只用20200709-subject-09这一个数据集进行训练！----------------------------------------------
            self.sample_list = {key: value for key, value in self.sample_dict.items() if value.get("color_file", "").startswith("20201015-subject-09")}
            # ------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def __getitem__(self,idx):
        sample = {}
        # print("----------------------------------idx = ",idx)
        # print("----------------------------------self.sample_list_processed[idx] = ",self.sample_list_processed[idx])
        # print("----------------------------------id_{idx} = ",f"id_{idx}")
        
        # print("---------------------------------len(self.sample_list_processed) = ",len(self.sample_list_processed))
        # print("---------------------------------self.sample_list_processed['id_0'] = ",self.sample_list_processed['id_0'])

        # 原文好像有问题（即train时，self.sample_list_processed为列表，而test时，self.sample_list_processed为一个字典即为self.sample_dict！），这里做了修改。
        if isinstance(self.sample_list_processed, list):
            sample_info = self.sample_dict[self.sample_list_processed[idx]]
        else:
            key = f"id_{idx}"
            value = self.sample_list_processed[key]
            sample_info = value
