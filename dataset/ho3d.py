...
# HO3D数据集的meta信息(即GT）：
        # xxx.pkl ----train集合下的信息~  ：=======？？要按树读取多级，而不是只是第一级
        #     objTrans: 物体的平移向量，表示物体在相机坐标系下的位置。
        #     handBeta: 手部的beta参数，用于生成手部的3D模型。
        #     camMat: 相机内参矩阵，包含相机的焦距、主点等信息。
        #     handJoints3D: 手部关节点在相机坐标系下的3D坐标。
        #     handTrans: 手部的平移向量，表示手部在相机坐标系下的位置。
        #     camIDList: 标记手部关节点所在相机的ID列表。
        #     handPose: 手部的姿态参数，用于生成手部的3D模型。
        #     objCorners3DRest: 物体的3D模型的八个顶点在物体本身坐标系下的坐标。
        #     objRot: 物体的旋转向量，表示物体的旋转角度和旋转轴。
        #     objName: 物体的名称，用于标识不同的物体。
        #     objCorners3D: 物体的8个顶点在相机坐标系下的3D坐标。
        #     objLabel: 物体的类别标签，用于区分不同的物体类别。

        # xxx.pkl ----evaluate集合下的信息~  ：

class HO3D(data.Dataset):
    def __init__(self, dataset_root, obj_model_root, train_label_root="E:/localPy/HOFEC/data/ho3d_v2",
        ...
        if self.mode == "train":
            ...
            # ==============================这些东西怎么来的？=========================================
            self.mano_params = [] # mano_pose + mano_shape ---<<meta中不知道咋处理得来的。。
            self.joints_uv = [] # 二维坐标（由3维坐标投影后得到） ---<<<需要的3d坐标，似乎是meta中的handJoints3D
            self.obj_p2ds = [] # 物体的二维坐标      ---<<<meta中好像只有3d坐标
            self.K = []  # sample["cam_intr"]  ---<<<即meta中的'camMat'相机的一些参数
            self.set_list  = [] # List<map<seqName,id>>，标识样本name与index的关系 ---<<<这个多加个seqName_id字段就ok了
            #data_ho3d = []
            with open(train_label_root + '/ho3d_train_data.json', 'r') as f:
                 data_ho3d = json.load(f)
            
            for data in data_ho3d:
                self.set_list.append(data['seqName_id']) # map<seqName,id>
                K = np.array(data['K'], dtype=np.float32)
                self.K.append(K)
                # ho3d_util.projectPoints: 将三维空间中的点投影到二维图像平面上
